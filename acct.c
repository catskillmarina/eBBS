...
_match_profile_key( char *rec, char *key )
{
  if (!strncmp(rec, key, 4) && rec[4] == '=') return S_RECEXISTS;
  return S_OK;
}

_profile_upd( char *newrec, char *oldrec, char *data )
{
...
_profile_format( char *rec, struct profile_data *pd )
{
  sprintf(rec, "%s=%s\n", pd->key, pd->data);
  return S_OK;
}
...
/*ARGSUSED*/
_profile_fill( int indx, char *rec, ACCOUNT *acct )
{
...
_fill_profile_name( char *rec, char *buf )
{
  strip_trailing_space(rec);
...
passent_to_account( char *rec, ACCOUNT *acct )
{
...
/*ARGSUSED*/
update_passent( char *newrec, char *oldrec, ACCOUNT *acct )
{
...
_enum_accounts( int indx, char *rec, struct enumstruct *en )
{
  ACCOUNT acct;
  memset(&acct, 0, sizeof acct);
...
_fill_acctnames( int indx, char *rec, struct listcomplete *lc )
{
  NAME userid;
